...
template <typename T>
typename BST<T>::display_rows BST<T>::get_row_display() const {
...
template <typename T>
std::vector<std::string> BST<T>::row_formatter(const display_rows& rows_disp) const {
...
template <typename T>
void BST<T>::trim_rows_left(std::vector<std::string>& rows) {
    if (!rows.size()) return;
...
template <typename T>
BST<T>::cell_display::cell_display() : present(false) {}

template <typename T>
BST<T>::cell_display::cell_display(std::string valstr) : valstr(valstr), present(true) {}
